...
void Bureaucrat::signForm(Form &form){
  if (form.getSigned() == true)
    std::cout << _Name << " cant sign " << form.getName() << " is already signed." << std::endl;
  else
    try {
      form.beSigned(*this);
      ...
      std::cout << _Name << " cant sign " << form.getName() << " because " << e.what() << std::endl;
    }
}
...
void  Bureaucrat::executeForm(Form const &form){
  try {
    ...
    std::cout << _Name << " cant executs " << form.getName() << " because " << e << std::endl;
  }
}
